...
function BeforeAfterCard({ item }: { item: TransformationPhoto }) {
  ...
  return (
    <div className="rounded-2xl bg-white overflow-hidden" style={{ boxShadow: "0 2px 16px rgba(0,0,0,0.06), 0 1px 3px rgba(0,0,0,0.04)" }}>
      <div className="p-5 border-b border-slate-100">
        <div className="flex items-center justify-between">
          <p className="text-sm font-black text-slate-800">{item.title || "Dönüşüm Hikayesi"}</p>
          <span className="rounded-full px-2.5 py-1 text-[10px] font-black uppercase tracking-wider" style={{ background: "rgba(249,115,22,0.1)", color: "#EA580C" }}>Önce/Sonra</span>
        </div>
      </div>
      ...
      <div className="relative h-72 overflow-hidden bg-slate-100">
        ...
        <div className="absolute left-3 top-3 rounded-full bg-black/70 px-2.5 py-1 text-[11px] font-black uppercase tracking-wider text-white">Sonra</div>
        <div className="absolute right-3 top-3 rounded-full bg-black/70 px-2.5 py-1 text-[11px] font-black uppercase tracking-wider text-white">Önce</div>
      </div>

      <div className="p-5 space-y-3">
        <div className="flex items-center justify-between">
          <span className="text-xs font-bold text-slate-500">Sürükle veya kaydır</span>
          <span className="text-xs font-black text-slate-800">{split}% Sonra</span>
        </div>
        <input
          type="range"
          min={0}
          ...
          className="w-full accent-orange-500 h-2 rounded"
        />
      </div>
    </div>
  );
